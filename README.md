...
In KO2, In order to add a color for each team shirt, luminosity of ball and player skin colors were changed so that skin is so bright and ball so reddish their color match. Then goalkeeper shirt light green was darkened so that it matched color for shadow. This way two extra color for each team shirt was available. 
